...
import { generateMerchandiseModal } from "../js/gallery.js"
...
async function renderMerchandise(type, name, order) {
    try {
        ...
        Merchandise.forEach((Merchandise) => {
            const letter = createMerchantLetter(Merchandise);
            listMerchandise.appendChild(letter);
        });
    } catch (error) {
